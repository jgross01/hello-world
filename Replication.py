...
# Requires NeighboursRecursive =====> HammingDistance =====>NumberToSymbol
	...
	for i in range(len(Text)-k+1):
		Pattern = Text[i:(i + k)]
		Neighbourhood = NeighboursRecursive(Pattern, d)
		for j in range(len(Neighbourhood)):
			if d == 0:
				FrequencyArray[PatternToNumber(Neighbourhood)] += 1
				break
			else:
				N = PatternToNumber(Neighbourhood[j])
...
#Incorporates Skew function without calling it
	L = len(Genome)
	SkewArray = [0]* (L+1)
	for i in range(L):
		if Genome[i] == "C":
...
def ApproxPatternCount(Text, Pattern, d):
	#Outputs the number of times input pattern occers in text with up to d mismatches
	#Requires HammingDist
	count = 0
	for i in range(len(Text)-len(Pattern)+1):
...
def FindingFrequentWordsBySorting(Text, k):
	#Another solution to finding frequent words.  Sorts each k-mer and then counts identicle k-mers in a row.
	FrequentPatterns = []
	Patterns = []
	Count =[]
	for i in range(len(Text) - k+1):
	...
	for i in range(len(Count)):
		if Count[i] == MaxCount:
			FrequentPatterns.append(Patterns[i])
	return FrequentPatterns

def FindingFrequentWordsBySortingWithMismatches(Text, k,d):
	#Another solution to finding frequent words.  Sorts each k-mer and then counts identicle k-mers in a row.
	FrequentPatterns = []
	Patterns = []
	Count =[]
	...
	for i in range(len(Text) - k+1):
		Neighbourhood = NeighboursRecursive(Text[i:k+i], d)
		for j in range(len(Neighbourhood)):
			Patterns.append(Neighbourhood[j])
			Count.append(1)
	Patterns.sort()
	for i in range(1,len(Count)):
		if Patterns[i]== Patterns[i-1]:
...
def FrequentPatternsWithMismatchesAndRC(Text,k,d):
	Count = [0] * (4**k)
	Frequent = []
	for i in range(len(Count)):
		Pattern = NumberToPattern(i,k)
		Count[i] = ApproxPatternCount(Text, Pattern,d) + ApproxPatternCount(Text ,ReverseCompliment(Pattern),d)

	MaxCount = max(Count)
	for i in range(len(Count)):
		if Count[i] == MaxCount:
			Frequent.append((NumberToPattern(i,k)))
	return Frequent
""" Code specific to finding dnaA boxes in Salmonella enterica
d=open("Salmonella_enterica.txt").readlines()
#remove the header
d=d[1:-1]
#join all lines
Text = "".join(map(str.strip, d))
k = 9
d = 1
d1=500
d2=500
FA = FrequencyArrayWithMismatches(Text[3764856-d1:3764856+d2],9, 1)
FARC = FrequencyArrayWithMismatches(ReverseCompliment(Text[3764856-d1:3764856+d2]),9, 1)
Tot = [0] * (4**k)
MostFrequent = []
...
for i in range(len(FA)):
	if Tot[i] == Max:
		MostFrequent.append(NumberToPattern(i,k))
